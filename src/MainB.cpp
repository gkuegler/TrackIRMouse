...
#define VERSION L"v0.4.0" // must be 12 characters or less


// create global objects
LPVOID lpSettingStructure = NULL;
...
int main(int argc, char* argv[]) {
	...
	// Get Windows environment and desktop info
	int iMonitorCount = WinSetup();

	// Populate the displays array with user bounds info
	// and other settings
	try
	{
		SConfig Conifg = LoadSettings(iMonitorCount);
	}
	...
	if (Conifg.bEnableWatchdog)
	{
		LPCSTR mxStartName = "Start";
		HANDLE mxStart = CreateMutexA(
			NULL,        // Security Descriptor, NULL sets default
			FALSE,       // bInitialOwner
			mxStartName  // lpName
		);
	...
	iRsltNP = NP_RegisterProgramProfileID(iProfileID);
