...
static const NpTitlesMap default_game_titles{
...
GetTitleIdsFromFile()
{
  ...
  try {
    return LoadJsonFromFileIntoObject<NpTitlesMap>(
...
{
  NpTitleList named(titles->size()), unnamed(titles->size());

  for (auto& [id, name] : *titles) {
    if (name.empty()) {
      unnamed.push_back({ id, name });
    } else {
      named.push_back({ id, name });
    }
  }

  std::sort(named.begin(), named.end(), [](const NpTitle& left, const NpTitle& right) {
    return left.name < right.name;
  });

  std::sort(unnamed.begin(), unnamed.end(), [](const NpTitle& left, const NpTitle& right) {
