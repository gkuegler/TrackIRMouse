...
NPRESULT NPClient_Init(LPTSTR pszDLLPath)
{
    ...
    if (NULL != ghNPClientDLL)
    {
        ...
        strcpy_s(verifySignature.DllSignature, 200,
                 ...
                 "Technologies");
        strcpy_s(
            verifySignature.AppSignature, 200,
        ...
        if (signature_result == NP_OK)
        {
                ...
                (strcmp(verifySignature.AppSignature, pSignature.AppSignature) == 0))
            {
                ...
                gpfNP_RegisterWindowHandle =
                    (PF_NP_REGISTERWINDOWHANDLE)::GetProcAddress(ghNPClientDLL, "NP_RegisterWindowHandle");
                gpfNP_UnregisterWindowHandle =
                    (PF_NP_UNREGISTERWINDOWHANDLE)::GetProcAddress(ghNPClientDLL, "NP_UnregisterWindowHandle");
                gpfNP_RegisterProgramProfileID =
                    (PF_NP_REGISTERPROGRAMPROFILEID)::GetProcAddress(ghNPClientDLL, "NP_RegisterProgramProfileID");
                ...
                gpfNP_StartDataTransmission =
                    (PF_NP_STARTDATATRANSMISSION)::GetProcAddress(ghNPClientDLL, "NP_StartDataTransmission");
                gpfNP_StopDataTransmission =
                    (PF_NP_STOPDATATRANSMISSION)::GetProcAddress(ghNPClientDLL, "NP_StopDataTransmission");
