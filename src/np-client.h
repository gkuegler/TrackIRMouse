...
//
// The following is the previous spec definition of versioning info -- I can
// probably do something very similar to this -- will keep you posted.
//
// request version information using 2 messages, they cannot be expected to
// arrive in a specific order - so always parse using the High byte the messages
// have a NPCONTROL byte in the first parameter, and the second parameter has
// packed bytes.
//   Message 1) (first parameter)NPCONTROL : (second parameter) (High
//   Byte)NPVERSIONMAJOR (Low Byte) major version number data Message 2) (first
//   parameter)NPCONTROL : (second parameter) (High Byte)NPVERSIONMINOR (Low
//   Byte) minor version number data
...
//
// Typedef for pointer to the notify callback function that is implemented
// within the client -- this function receives head tracker reports from the
// game client API
//
typedef NPRESULT(__stdcall *PF_NOTIFYCALLBACK)(unsigned short, unsigned short);

// Typedefs for game client API functions (useful for declaring pointers to
// these functions within the client for use during GetProcAddress() ops)
//
...
//// Function Prototypes ///////////////////////////////////////////////
//
// Functions exported from game client API DLL ( note __stdcall calling
// convention is used for ease of interface to clients of differing
// implementations including C, C++, Pascal (Delphi) and VB. )
//
